...
model_ids = [
    ('microsoft/Phi-4-mini-reasoning', f'{cache_dir}/models--microsoft--Phi-4-mini-reasoning/snapshots/0e3b1e2d02ee478a3743abe3f629e9c0cb722e0a')
]
    ...
    for thinking_allowed in [True, False]:
        for _, row in df.iterrows():
            ...
            if thinking_allowed:
                ...
                token_tensor = torch.tensor([tokenizer.encode('<|assistant|> <think></think>\n\n')], device=input_data['input_ids'].device, dtype=input_data['input_ids'].dtype)
                input_ids = torch.cat([input_data['input_ids'], token_tensor], dim=1)
                input_data['input_ids'] = input_ids
                input_data['attention_mask'] = torch.ones_like(input_data['input_ids'])
