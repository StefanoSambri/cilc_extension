...
model_ids = [
    ('zai-org/GLM-4.1V-9B-Thinking', f'{cache_dir}/models--zai-org--GLM-4.1V-9B-Thinking/snapshots/9e9a4c5e94f4a095c353f4152d520a2644a553b2')
]
    ...
    for thinking_allowed in [True, False]:
        for idx, row in df.iterrows():
            ...
            if thinking_allowed:
                ...
                token_tensor = torch.tensor([tokenizer.encode(think_content + '<answer>')], device=input_data['input_ids'].device, dtype=input_data['input_ids'].dtype)
                ...
                trace_len = None
                input_data = processor.apply_chat_template(messages, tokenize=True, add_generation_prompt=True, return_dict=True, return_tensors='pt').to(device=model.device)
                token_tensor = torch.tensor([tokenizer.encode('<think></think><answer>')], device=input_data['input_ids'].device, dtype=input_data['input_ids'].dtype)
                input_ids = torch.cat([input_data['input_ids'], token_tensor], dim=1)
                input_data['input_ids'] = input_ids
                input_data['attention_mask'] = torch.ones_like(input_data['input_ids'])
